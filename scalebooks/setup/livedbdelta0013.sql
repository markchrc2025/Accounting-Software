...
-- ── Verify ───────────────────────────────────────────────────────────────────
-- Expect: org_settings / checkbooks / check_registry / disbursement_reports all
-- non-null, and vouchers_0013_ok = 2.
SELECT
  to_regclass('public.org_settings')         AS org_settings_ok,
  to_regclass('public.checkbooks')           AS checkbooks_ok,
  to_regclass('public.check_registry')       AS check_registry_ok,
  to_regclass('public.disbursement_reports') AS disbursement_reports_ok,
  (SELECT count(*) FROM information_schema.columns
    WHERE table_name = 'vouchers'
      AND column_name IN ('pre_disbursement_status','disbursement_ref')) AS vouchers_0013_ok;
