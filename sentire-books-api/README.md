...
| Validation | Zod (shared FE ↔ BE via `@sentire-books-api/domain`) |
...
- **Exact money.** `bigint` centavos in the DB and integer math in `@sentire-books-api/domain`
...
pnpm --filter @sentire-books-api/api dev
pnpm --filter @sentire-books-api/web dev
```
