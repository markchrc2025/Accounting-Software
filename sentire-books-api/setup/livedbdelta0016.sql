...
-- ── Verify ───────────────────────────────────────────────────────────────────
-- Expect: all eight columns non-null (each shows its table name).
SELECT
  to_regclass('public.loans')                      AS loans_ok,
  to_regclass('public.loan_payments')              AS loan_payments_ok,
  to_regclass('public.fixed_assets')               AS fixed_assets_ok,
  to_regclass('public.asset_types')                AS asset_types_ok,
  to_regclass('public.asset_installment_payments') AS asset_installment_payments_ok,
  to_regclass('public.asset_depr_postings')        AS asset_depr_postings_ok,
  to_regclass('public.weekly_projections')         AS weekly_projections_ok,
  to_regclass('public.credit_lines')               AS credit_lines_ok;
