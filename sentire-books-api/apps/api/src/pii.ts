...
/** Minimal shapes — avoids depending on Sentry's types in the portal copy. */
 ...
 */
export function scrubBreadcrumb(bc: ScrubbableBreadcrumb | null): ScrubbableBreadcrumb | null {
  if (!bc) return null;
  if (bc.category === "console") return null;

  const out: ScrubbableBreadcrumb = { ...bc };
  ...
  if (out.data) {
    ...
    out.data = scrubDeep(data) as Record<string, unknown>;
  }
  return out;
}
